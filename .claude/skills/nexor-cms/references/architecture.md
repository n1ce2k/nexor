...
Инфоблок с включённым `has_page` владеет папкой `resources/views/<код>/index.blade.php` — как папка-страница в Битриксе. Файл создаёт `Nexor\Cms\Support\PageGenerator` при сохранении и **никогда не перезаписывает**: дальше он принадлежит тому, кто его правит. Путь запоминается в `iblocks.page_path`.

Публичный роут `/{code}` сначала ищет такую страницу и только потом — элемент инфоблока «Страницы».
