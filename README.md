...
**16 типов свойств:** строка, текст, HTML, целое, дробное, да/нет, дата, дата и время, цвет, список, файл, изображение, привязка к элементу, к разделу, к пользователю, JSON. Флаги: множественное, обязательное, в фильтре, в поиске, колонкой в списке. У свойства есть «Подсказка для редактора» (короткая строка под полем) и **«Описание»** под значением по умолчанию — текст подлиннее, который видит редактор в форме элемента и получают шаблоны сайта: `$property->description`. Значения лежат в типизированной EAV-таблице с индексами под фильтрацию.
