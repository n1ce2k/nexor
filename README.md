...
**Фильтр и сортировка по цене.** Цена лежит не в свойствах, а в торговом каталоге, поэтому у неё свои параметры — `?price_from=` и `?price_to=`; `catalog.section` читает их сам. Сортировка — `:order="['price' => 'asc']"`. Считается цена, которую видит покупатель: со скидкой, а у товара с торговыми предложениями — лучшая из них, так что товар находится по цене своих предложений.

`catalog.filter` сам добавляет блок «Цена» с ползунком «от и до»: границы берутся из активных товаров инфоблока, а на странице раздела — из его товаров. Все товары стоят одинаково или цен нет вовсе — блока не будет. Выключить: `<x-nexor::catalog.filter iblock="katalog" :price="false" />`. В шаблон приходит `$priceRange` — `['min', 'max', 'from', 'to']` или `null`.

Границы работают вместе: `['price' => ['from' => 1000, 'to' => 2000]]` в фильтре сервиса и `?CODE_FROM=&CODE_TO=` у числовых свойств — раньше верхняя граница затирала нижнюю.
...
| `catalog.filter` | `catalog.filter` | фильтр по свойствам и цене |
