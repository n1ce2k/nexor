...
`nexor:install` проводит миграции, создаёт роли и администратора, готовит модель пользователя приложения (спрашивает разрешение и дописывает контракт `NexorUser`, трейты `HasRoles` и `HasUserFields` и колонки CMS в список заполняемых), кладёт в `resources/views/site` стартовые шаблоны сайта (только недостающие — свои не перезапишет) и дописывает в `resources/css/app.css` строки `@source` на шаблоны компонентов пакета, чтобы Tailwind сайта видел их классы.
